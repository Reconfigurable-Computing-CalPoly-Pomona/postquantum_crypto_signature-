...
void hss_hash_ctx(void *result, int hash_type, union hash_context *ctx,
	...
	//***begin edit***
	case HASH_SHA3: {
		sha3_init(&ctx->sha3, 32);
		sha3_update(&ctx->sha3, message, message_len);
		sha3_final(result, &ctx->sha3);
    #if ALLOW_VERBOSE
            if (hss_verbose) {
                printf( " ->" );
                int i; for (i=0; i<32; i++) printf( " %02x", ((unsigned char *)result)[i] ); printf( "\n" );
...
void hss_update_hash_context(int h, union hash_context *ctx,
	...
	//***begin edit***
    case HASH_SHA3:
		    sha3_update(&ctx->sha3, msg, len_msg);
        break;
	//***end edit***
    }
}
...
unsigned hss_hash_length(int hash_type) {
    ...
    case HASH_SHA3: return 32;		//***edited***
    }
    return 0;
}
...
unsigned hss_hash_blocksize(int hash_type) {
    ...
    case HASH_SHA3: return 136;			//***edited*** 1088 bit
    }
    return 0;
}
